...
describe('SequelizeQueryService', () => {
    let moduleRef;
    let TestEntityService = class TestEntityService extends src_1.SequelizeQueryService {
        constructor(model) {
            super(model);
            this.model = model;
        }
    ...
    TestEntityService = (0, tslib_1.__decorate)([
        (0, tslib_1.__param)(0, (0, sequelize_1.InjectModel)(test_entity_1.TestEntity)),
        (0, tslib_1.__metadata)("design:paramtypes", [Object])
    ], TestEntityService);
    let TestRelationService = class TestRelationService extends src_1.SequelizeQueryService {
        constructor(model) {
            super(model);
            this.model = model;
        }
    ...
    TestRelationService = (0, tslib_1.__decorate)([
        (0, tslib_1.__param)(0, (0, sequelize_1.InjectModel)(test_relation_entity_1.TestRelation)),
        (0, tslib_1.__metadata)("design:paramtypes", [Object])
    ], TestRelationService);
    ...
    beforeEach(async () => {
        moduleRef = await testing_1.Test.createTestingModule({
            imports: [
                sequelize_1.SequelizeModule.forRoot(sequelize_fixture_1.CONNECTION_OPTIONS),
                sequelize_1.SequelizeModule.forFeature([test_entity_1.TestEntity, test_relation_entity_1.TestRelation, test_entity_test_relation_entity_1.TestEntityTestRelationEntity]),
            ],
            providers: [TestEntityService, TestRelationService],
        }).compile();
        const sequelize = moduleRef.get(sequelize_typescript_1.Sequelize);
        await sequelize.sync();
        await (0, sequelize_fixture_1.refresh)(sequelize);
    });
    ...
    describe('#query', () => {
        it('call select and return the result', async () => {
            ...
            return expect(queryResult.map((e) => e.get({ plain: true }))).toEqual([seeds_1.PLAIN_TEST_ENTITIES[0]]);
        });
        describe('filter on relations', () => {
            describe('oneToOne', () => {
                it('should allow filtering on a one to one relation', async () => {
                    const entity = seeds_1.PLAIN_TEST_ENTITIES[0];
                    const queryService = moduleRef.get(TestEntityService);
                ...
                it('should allow filtering on a many to one relation', async () => {
                    ...
                    const queryResults = await queryService.query({
                        filter: {
                            testEntity: {
                                testEntityPk: {
                                    in: [seeds_1.PLAIN_TEST_ENTITIES[0].testEntityPk, seeds_1.PLAIN_TEST_ENTITIES[1].testEntityPk],
                                },
                            },
                        },
                    });
                    expect(queryResults.map((e) => e.get({ plain: true }))).toEqual(seeds_1.PLAIN_TEST_RELATIONS.slice(0, 6));
                });
            });
            describe('oneToMany', () => {
                it('should allow filtering on a many to one relation', async () => {
                    ...
                    const queryResult = await queryService.query({
                        filter: {
                            testRelations: {
                                relationName: {
                                    in: [seeds_1.PLAIN_TEST_RELATIONS[0].relationName, seeds_1.PLAIN_TEST_RELATIONS[1].relationName],
                                },
                            },
                        },
                    });
    ...
    describe('#count', () => {
        ...
        describe('with relations', () => {
            ...
            describe('manyToOne', () => {
                it('set the relation to null', async () => {
                    ...
                    const count = await queryService.count({
                        testEntity: {
                            testEntityPk: {
                                in: [seeds_1.PLAIN_TEST_ENTITIES[0].testEntityPk, seeds_1.PLAIN_TEST_ENTITIES[2].testEntityPk],
                            },
                        },
                    });
                    expect(count).toBe(6);
            ...
            describe('oneToMany', () => {
                it('set the relation to null', async () => {
                    const relation = seeds_1.PLAIN_TEST_RELATIONS[0];
                    const queryService = moduleRef.get(TestEntityService);
                    const count = await queryService.count({
                        testRelations: {
                            testEntityId: {
    ...
    describe('#queryRelations', () => {
        describe('with one entity', () => {
            it('call select and return the result', async () => {
                const queryService = moduleRef.get(TestEntityService);
                const queryResult = await queryService.queryRelations(test_relation_entity_1.TestRelation, 'testRelations', test_entity_1.TestEntity.build(seeds_1.PLAIN_TEST_ENTITIES[0]), {
                    filter: { relationName: { isNot: null } },
                });
                return expect(queryResult.map((r) => r.testEntityId)).toEqual([
                    seeds_1.PLAIN_TEST_ENTITIES[0].testEntityPk,
                    seeds_1.PLAIN_TEST_ENTITIES[0].testEntityPk,
                    seeds_1.PLAIN_TEST_ENTITIES[0].testEntityPk,
                ]);
            });
        ...
        describe('with multiple entities', () => {
            it('call select and return the result', async () => {
                const entities = seeds_1.PLAIN_TEST_ENTITIES.slice(0, 3).map((pe) => test_entity_1.TestEntity.build(pe));
                const queryService = moduleRef.get(TestEntityService);
    ...
    describe('#aggregateRelations', () => {
        describe('with one entity', () => {
            it('call select and return the result', async () => {
                const queryService = moduleRef.get(TestEntityService);
                const aggResult = await queryService.aggregateRelations(test_relation_entity_1.TestRelation, 'testRelations', test_entity_1.TestEntity.build(seeds_1.PLAIN_TEST_ENTITIES[0]), { relationName: { isNot: null } }, { count: ['testRelationPk'] });
    ...
    describe('#countRelations', () => {
        describe('with one entity', () => {
            it('call count and return the result', async () => {
                const queryService = moduleRef.get(TestEntityService);
                const entity = test_entity_1.TestEntity.build(seeds_1.PLAIN_TEST_ENTITIES[0]);
                const countResult = await queryService.countRelations(test_relation_entity_1.TestRelation, 'testRelations', entity, {
    ...
    describe('#findRelation', () => {
        describe('with one entity', () => {
            it('call select and return the result', async () => {
                ...
                expect(queryResult.get({ plain: true })).toEqual(seeds_1.PLAIN_TEST_RELATIONS[0]);
            });
            it('apply the filter option', async () => {
                ...
                const queryResult1 = await queryService.findRelation(test_relation_entity_1.TestRelation, 'oneTestRelation', entity, {
                    filter: { relationName: { eq: seeds_1.PLAIN_TEST_RELATIONS[0].relationName } },
                });
                expect(queryResult1.get({ plain: true })).toEqual(seeds_1.PLAIN_TEST_RELATIONS[0]);
                const queryResult2 = await queryService.findRelation(test_relation_entity_1.TestRelation, 'oneTestRelation', entity, {
                    filter: { relationName: { eq: seeds_1.PLAIN_TEST_RELATIONS[1].relationName } },
                });
                expect(queryResult2).toBeUndefined();
            });
            it('should return undefined select if no results are found.', async () => {
                const entity = { ...seeds_1.PLAIN_TEST_ENTITIES[0], testEntityPk: 'not-real' };
                const queryService = moduleRef.get(TestEntityService);
                const queryResult = await queryService.findRelation(test_relation_entity_1.TestRelation, 'oneTestRelation', entity);
            ...
            it('throw an error if a relation with that name is not found.', async () => {
                const queryService = moduleRef.get(TestEntityService);
                const entity = test_entity_1.TestEntity.build(seeds_1.PLAIN_TEST_ENTITIES[0]);
                return expect(queryService.findRelation(test_relation_entity_1.TestRelation, 'badRelation', entity)).rejects.toThrow('Unable to find relation badRelation on TestEntity');
            });
        });
        describe('with multiple entities', () => {
            it('call select and return the result', async () => {
                ...
                expect(queryResult).toEqual(new Map([
                    [entities[0], expect.objectContaining(seeds_1.PLAIN_TEST_RELATIONS[0])],
                    [entities[1], expect.objectContaining(seeds_1.PLAIN_TEST_RELATIONS[3])],
                    [entities[2], expect.objectContaining(seeds_1.PLAIN_TEST_RELATIONS[6])],
                ]));
            });
            it('should apply the filter option', async () => {
                ...
                const queryResult = await queryService.findRelation(test_relation_entity_1.TestRelation, 'oneTestRelation', entities, {
                    filter: {
                        testRelationPk: { in: [seeds_1.PLAIN_TEST_RELATIONS[0].testRelationPk, seeds_1.PLAIN_TEST_RELATIONS[6].testRelationPk] },
                    },
                });
                expect(queryResult).toEqual(new Map([
                    [entities[0], expect.objectContaining(seeds_1.PLAIN_TEST_RELATIONS[0])],
            ...
            it('should return undefined select if no results are found.', async () => {
                ...
                const queryService = moduleRef.get(TestEntityService);
                const queryResult = await queryService.findRelation(test_relation_entity_1.TestRelation, 'oneTestRelation', entities);
                expect(queryResult).toEqual(new Map([[entities[0], expect.objectContaining(seeds_1.PLAIN_TEST_RELATIONS[0])]]));
            });
        });
    });
    describe('#addRelations', () => {
        it('call select and return the result', async () => {
            const entity = seeds_1.PLAIN_TEST_ENTITIES[0];
            const queryService = moduleRef.get(TestEntityService);
            const queryResult = await queryService.addRelations('testRelations', entity.testEntityPk, seeds_1.PLAIN_TEST_RELATIONS.slice(3, 6).map((r) => r.testRelationPk));
        ...
        describe('with modify options', () => {
            it('should throw an error if the entity is not found with the id and provided filter', async () => {
                const entity = seeds_1.PLAIN_TEST_ENTITIES[0];
                const queryService = moduleRef.get(TestEntityService);
                return expect(queryService.addRelations('testRelations', entity.testEntityPk, seeds_1.PLAIN_TEST_RELATIONS.slice(3, 6).map((r) => r.testRelationPk), {
                    filter: { stringType: { eq: seeds_1.PLAIN_TEST_ENTITIES[1].stringType } },
    ...
    describe('#setRelations', () => {
        it('set all relations on the entity', async () => {
            const entity = seeds_1.PLAIN_TEST_ENTITIES[0];
            const queryService = moduleRef.get(TestEntityService);
            const relationIds = seeds_1.PLAIN_TEST_RELATIONS.slice(3, 6).map((r) => r.testRelationPk);
        ...
        describe('with modify options', () => {
            it('should throw an error if the entity is not found with the id and provided filter', async () => {
                const entity = seeds_1.PLAIN_TEST_ENTITIES[0];
                const queryService = moduleRef.get(TestEntityService);
                return expect(queryService.setRelations('testRelations', entity.testEntityPk, seeds_1.PLAIN_TEST_RELATIONS.slice(3, 6).map((r) => r.testRelationPk), {
                    filter: { stringType: { eq: seeds_1.PLAIN_TEST_ENTITIES[1].stringType } },
                })).rejects.toThrow('Unable to find TestEntity with id: test-entity-1');
            });
            it('should throw an error if the relations are not found with the relationIds and provided filter', async () => {
    ...
    describe('#setRelation', () => {
        it('call select and return the result', async () => {
            const entity = seeds_1.PLAIN_TEST_ENTITIES[0];
            const queryService = moduleRef.get(TestEntityService);
            const queryResult = await queryService.setRelation('oneTestRelation', entity.testEntityPk, seeds_1.PLAIN_TEST_RELATIONS[1].testRelationPk);
            expect(queryResult).toEqual(expect.objectContaining(entity));
            const relation = await queryService.findRelation(test_relation_entity_1.TestRelation, 'oneTestRelation', entity);
            expect(relation.testRelationPk).toBe(seeds_1.PLAIN_TEST_RELATIONS[1].testRelationPk);
        });
        describe('with modify options', () => {
            it('should throw an error if the entity is not found with the id and provided filter', async () => {
                const entity = seeds_1.PLAIN_TEST_ENTITIES[0];
                const queryService = moduleRef.get(TestEntityService);
                return expect(queryService.setRelation('oneTestRelation', entity.testEntityPk, seeds_1.PLAIN_TEST_RELATIONS[1].testRelationPk, {
                    filter: { stringType: { eq: seeds_1.PLAIN_TEST_ENTITIES[1].stringType } },
                })).rejects.toThrow('Unable to find TestEntity with id: test-entity-1');
            });
            it('should throw an error if the relations are not found with the relationIds and provided filter', async () => {
    ...
    describe('#removeRelations', () => {
        it('call select and return the result', async () => {
            const entity = seeds_1.PLAIN_TEST_ENTITIES[0];
            const queryService = moduleRef.get(TestEntityService);
            const queryResult = await queryService.removeRelations('testRelations', entity.testEntityPk, seeds_1.PLAIN_TEST_RELATIONS.slice(0, 3).map((r) => r.testRelationPk));
            expect(queryResult).toEqual(expect.objectContaining(entity));
            const relations = await queryService.queryRelations(test_relation_entity_1.TestRelation, 'testRelations', entity, {});
        ...
        describe('with modify options', () => {
            it('should throw an error if the entity is not found with the id and provided filter', async () => {
                const entity = seeds_1.PLAIN_TEST_ENTITIES[0];
                const queryService = moduleRef.get(TestEntityService);
                return expect(queryService.removeRelations('testRelations', entity.testEntityPk, seeds_1.PLAIN_TEST_RELATIONS.slice(3, 6).map((r) => r.testRelationPk), {
                    filter: { stringType: { eq: seeds_1.PLAIN_TEST_ENTITIES[1].stringType } },
                })).rejects.toThrow('Unable to find TestEntity with id: test-entity-1');
            });
            it('should throw an error if the relations are not found with the relationIds and provided filter', async () => {
    ...
    describe('#removeRelation', () => {
        describe('oneToOne', () => {
            it('set the relation to null', async () => {
                const entity = seeds_1.PLAIN_TEST_ENTITIES[0];
                const queryService = moduleRef.get(TestEntityService);
                const queryResult = await queryService.removeRelation('oneTestRelation', entity.testEntityPk, seeds_1.PLAIN_TEST_RELATIONS[0].testRelationPk);
                expect(queryResult).toEqual(expect.objectContaining(entity));
                const relation = await queryService.findRelation(test_relation_entity_1.TestRelation, 'oneTestRelation', entity);
            ...
            describe('with modify options', () => {
                it('should throw an error if the entity is not found with the id and provided filter', async () => {
                    const entity = seeds_1.PLAIN_TEST_ENTITIES[0];
                    const queryService = moduleRef.get(TestEntityService);
                    return expect(queryService.removeRelation('oneTestRelation', entity.testEntityPk, seeds_1.PLAIN_TEST_RELATIONS[1].testRelationPk, {
                        filter: { stringType: { eq: seeds_1.PLAIN_TEST_ENTITIES[1].stringType } },
                    })).rejects.toThrow('Unable to find TestEntity with id: test-entity-1');
                });
                it('should throw an error if the relations are not found with the relationIds and provided filter', async () => {
        ...
        describe('manyToOne', () => {
            it('set the relation to null', async () => {
                const relation = seeds_1.PLAIN_TEST_RELATIONS[0];
                const queryService = moduleRef.get(TestRelationService);
                const queryResult = await queryService.removeRelation('testEntity', relation.testRelationPk, seeds_1.PLAIN_TEST_ENTITIES[0].testEntityPk);
                expect(queryResult).toEqual(expect.objectContaining({ ...relation, testEntityId: null }));
                const entity = await queryService.findRelation(test_entity_1.TestEntity, 'testEntity', queryResult);
            ...
            describe('with modify options', () => {
                it('should throw an error if the entity is not found with the id and provided filter', async () => {
                    const relation = seeds_1.PLAIN_TEST_RELATIONS[0];
                    const queryService = moduleRef.get(TestRelationService);
                    return expect(queryService.removeRelation('testEntity', relation.testRelationPk, seeds_1.PLAIN_TEST_ENTITIES[1].testEntityPk, {
                        filter: { relationName: { eq: seeds_1.PLAIN_TEST_RELATIONS[1].relationName } },
                ...
                it('should throw an error if the relations are not found with the relationIds and provided filter', async () => {
                    const relation = seeds_1.PLAIN_TEST_RELATIONS[0];
                    const queryService = moduleRef.get(TestRelationService);
                    return expect(queryService.removeRelation('testEntity', relation.testRelationPk, seeds_1.PLAIN_TEST_ENTITIES[0].testEntityPk, {
                        relationFilter: { stringType: { eq: seeds_1.PLAIN_TEST_ENTITIES[1].stringType } },
        ...
        describe('oneToMany', () => {
            it('set the relation to null', async () => {
                const entity = seeds_1.PLAIN_TEST_ENTITIES[0];
                const queryService = moduleRef.get(TestEntityService);
                const queryResult = await queryService.removeRelation('testRelations', entity.testEntityPk, seeds_1.PLAIN_TEST_RELATIONS[0].testRelationPk);
                expect(queryResult).toEqual(expect.objectContaining(entity));
                const relations = await queryService.queryRelations(test_relation_entity_1.TestRelation, 'testRelations', entity, {});
            ...
            describe('with modify options', () => {
                it('should throw an error if the entity is not found with the id and provided filter', async () => {
                    const entity = seeds_1.PLAIN_TEST_ENTITIES[0];
                    const queryService = moduleRef.get(TestEntityService);
                    return expect(queryService.removeRelation('testRelations', entity.testEntityPk, seeds_1.PLAIN_TEST_RELATIONS[4].testRelationPk, {
                        filter: { stringType: { eq: seeds_1.PLAIN_TEST_ENTITIES[1].stringType } },
                    })).rejects.toThrow('Unable to find TestEntity with id: test-entity-1');
                });
                it('should throw an error if the relations are not found with the relationIds and provided filter', async () => {
    ...
    describe('#createMany', () => {
        it('call save on the repo with instances of entities when passed plain objects', async () => {
            await (0, sequelize_fixture_1.truncate)(moduleRef.get(sequelize_typescript_1.Sequelize));
            const queryService = moduleRef.get(TestEntityService);
            const created = await queryService.createMany(seeds_1.PLAIN_TEST_ENTITIES);
            expect(created.map((c) => c.get({ plain: true }))).toEqual(expect.objectContaining(seeds_1.PLAIN_TEST_ENTITIES));
        });
        it('call save on the repo with instances of entities when passed instances', async () => {
            await (0, sequelize_fixture_1.truncate)(moduleRef.get(sequelize_typescript_1.Sequelize));
            const queryService = moduleRef.get(TestEntityService);
            const entities = seeds_1.PLAIN_TEST_ENTITIES.map((pe) => test_entity_1.TestEntity.build(pe));
            const created = await queryService.createMany(entities);
            expect(created.map((c) => c.get({ plain: true }))).toEqual(seeds_1.PLAIN_TEST_ENTITIES);
        });
        it('should reject if the entities already exist', async () => {
            const queryService = moduleRef.get(TestEntityService);
            return expect(queryService.createMany(seeds_1.PLAIN_TEST_ENTITIES)).rejects.toThrow('Entity already exists');
        });
    });
    describe('#createOne', () => {
    ...
    describe('#deleteMany', () => {
        it('delete all records that match the query', async () => {
            ...
            const { deletedCount } = await queryService.deleteMany({
                testEntityPk: { in: seeds_1.PLAIN_TEST_ENTITIES.slice(0, 5).map((e) => e.testEntityPk) },
            });
    ...
    describe('#deleteOne', () => {
        it('remove the entity', async () => {
            const queryService = moduleRef.get(TestEntityService);
            const deleted = await queryService.deleteOne(seeds_1.PLAIN_TEST_ENTITIES[0].testEntityPk);
            expect(deleted).toEqual(expect.objectContaining(seeds_1.PLAIN_TEST_ENTITIES[0]));
        });
    ...
    describe('#updateOne', () => {
        it('update the entity', async () => {
            const queryService = moduleRef.get(TestEntityService);
            const updated = await queryService.updateOne(seeds_1.PLAIN_TEST_ENTITIES[0].testEntityPk, { stringType: 'updated' });
            expect(updated).toEqual(expect.objectContaining({ ...seeds_1.PLAIN_TEST_ENTITIES[0], stringType: 'updated' }));
        });
        it('should reject if the update contains a primary key', async () => {
            const queryService = moduleRef.get(TestEntityService);
            return expect(queryService.updateOne(seeds_1.PLAIN_TEST_ENTITIES[0].testEntityPk, { testEntityPk: 'bad-id' })).rejects.toThrow('Id cannot be specified when updating');
        });
        it('call fail if the entity is not found', async () => {
            const queryService = moduleRef.get(TestEntityService);
        ...
        describe('with filter', () => {
            ...
            it('should throw an error if unable to find the entity', async () => {
                const entity = seeds_1.PLAIN_TEST_ENTITIES[0];
                const queryService = moduleRef.get(TestEntityService);
                return expect(queryService.updateOne(entity.testEntityPk, { stringType: 'updated' }, { filter: { stringType: { eq: seeds_1.PLAIN_TEST_ENTITIES[1].stringType } } })).rejects.toThrow(`Unable to find TestEntity with id: ${entity.testEntityPk}`);
            });
        });
    });
});
//# sourceMappingURL=sequelize-query.service.spec.js.map