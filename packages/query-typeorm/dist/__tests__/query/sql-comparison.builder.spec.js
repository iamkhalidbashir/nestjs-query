...
describe('SQLComparisonBuilder', () => {
    ...
    beforeEach(() => {
        let index = -1;
        common_1.randomString.mockImplementation(() => {
            index += 1;
    ...
    describe('eq comparisons', () => {
        it('should build a qualified eq sql fragment', () => {
    ...
    describe('is comparisons', () => {
        ...
        it('should throw an error for values other than null true or false', () => {
            // @ts-ignore
            expect(() => createSQLComparisonBuilder().build('boolType', 'is', 'foo', 'TestEntity')).toThrow('unexpected is operator param "foo"');
        });
    });
    describe('isNot comparisons', () => {
        ...
        it('should throw an error for values other than null true or false', () => {
            // @ts-ignore
            expect(() => createSQLComparisonBuilder().build('boolType', 'isNot', 'foo', 'TestEntity')).toThrow('unexpected isNot operator param "foo"');
        });
    });
    describe('in comparisons', () => {
        ...
        it('should throw an error for empty array', () => {
            const arr = [];
            expect(() => createSQLComparisonBuilder().build('numberType', 'in', arr, 'TestEntity')).toThrow('Invalid in value expected a non-empty array got []');
        });
        it('should throw an error for non-array', () => {
            expect(() => createSQLComparisonBuilder().build('numberType', 'in', 1, 'TestEntity')).toThrow('Invalid in value expected an array got 1');
        });
    });
    describe('notIn comparisons', () => {
        ...
        it('should throw an error for empty array', () => {
            const arr = [];
            expect(() => createSQLComparisonBuilder().build('numberType', 'notIn', arr, 'TestEntity')).toThrow('Invalid in value expected a non-empty array got []');
        });
        it('should throw an error for non-array', () => {
            expect(() => createSQLComparisonBuilder().build('numberType', 'notIn', 1, 'TestEntity')).toThrow('Invalid in value expected an array got 1');
        });
    });
