...
describe('TypegooseQueryService', () => {
    ...
    TestEntityService = (0, tslib_1.__decorate)([
        (0, tslib_1.__param)(0, (0, nestjs_typegoose_1.InjectModel)(__fixtures__1.TestEntity)),
    ...
    describe('#createOne', () => {
        ...
        it('call save on the repo with an instance of the entity when passed an instance', async () => {
            const Model = (0, typegoose_1.getModelForClass)(__fixtures__1.TestEntity);
    ...
    describe('#updateMany', () => {
        ...
        it('should not reject if the update contains an undefined id', async () => {
            const queryService = moduleRef.get(TestEntityService);
            const entitiesToUpdate = __fixtures__1.TEST_ENTITIES.slice(0, 5);
            const filter = {
    ...
    describe('#updateOne', () => {
        ...
        it('should reject if the update contains the ID', async () => {
            const queryService = moduleRef.get(TestEntityService);
            return expect(queryService.updateOne(__fixtures__1.TEST_ENTITIES[0]._id.toHexString(), { _id: new Types.ObjectId() })).rejects.toThrow('Id cannot be specified when updating');
        });
        it('call fail if the entity is not found', async () => {
            const badId = new Types.ObjectId().toHexString();
            const queryService = moduleRef.get(TestEntityService);
    ...
    describe('#findRelation', () => {
        describe('with one entity', () => {
            it('call select and return the result', async () => {
                ...
                expect(queryResult).toEqual(__fixtures__1.TEST_REFERENCES[0]);
            });
            it('apply the filter option', async () => {
                ...
                expect(queryResult1).toEqual(__fixtures__1.TEST_REFERENCES[0]);
            ...
            describe('virtual reference', () => {
                it('call select and return the result', async () => {
                    ...
                    expect(queryResult).toEqual(__fixtures__1.TEST_ENTITIES[0]);
                });
                it('apply the filter option', async () => {
                    const entity = __fixtures__1.TEST_REFERENCES[0];
                    const queryService = moduleRef.get(TestReferenceService);
                    ...
                    expect(queryResult1).toEqual(__fixtures__1.TEST_ENTITIES[0]);
        ...
        describe('with multiple entities', () => {
            ...
            it('should apply the filter option', async () => {
                ...
                const options = {
                    filter: {
                        _id: { in: [__fixtures__1.TEST_REFERENCES[0]._id, __fixtures__1.TEST_REFERENCES[6]._id] },
                    },
                };
    ...
    describe('#queryRelations', () => {
        describe('with one entity', () => {
            it('call select and return the result', async () => {
                ...
                return expect(queryResult).toEqual(__fixtures__1.TEST_REFERENCES.slice(0, 3));
            });
            it('should apply a filter', async () => {
                ...
                expect(queryResult).toEqual([__fixtures__1.TEST_REFERENCES[1]]);
            });
            it('should apply paging', async () => {
                ...
                expect(queryResult).toEqual(__fixtures__1.TEST_REFERENCES.slice(1, 3));
            });
        });
        describe('with virtual entity', () => {
            it('call select and return the result', async () => {
                ...
                return expect(queryResult).toEqual(expect.arrayContaining(__fixtures__1.TEST_REFERENCES.slice(0, 3)));
            });
            it('should apply a filter', async () => {
                const queryService = moduleRef.get(TestEntityService);
                const queryResult = await queryService.queryRelations(__fixtures__1.TestReference, 'virtualTestReferences', __fixtures__1.TEST_ENTITIES[0], {
        ...
        describe('with multiple entities', () => {
            it('call return a map of results for each entity', async () => {
                ...
                expect(queryResult.get(entities[0])).toEqual(__fixtures__1.TEST_REFERENCES.slice(0, 3));
                expect(queryResult.get(entities[1])).toEqual(__fixtures__1.TEST_REFERENCES.slice(3, 6));
                expect(queryResult.get(entities[2])).toEqual(__fixtures__1.TEST_REFERENCES.slice(6, 9));
            });
            ...
            it('should apply paging per entity', async () => {
                ...
                expect(queryResult.size).toBe(3);
                expect(queryResult.get(entities[0])).toEqual(__fixtures__1.TEST_REFERENCES.slice(1, 3));
                expect(queryResult.get(entities[1])).toEqual(__fixtures__1.TEST_REFERENCES.slice(4, 6));
                expect(queryResult.get(entities[2])).toEqual(__fixtures__1.TEST_REFERENCES.slice(7, 9));
            });
    ...
    describe('#setRelations', () => {
        ...
        describe('with modify options', () => {
            it('should throw an error if the entity is not found with the id and provided filter', async () => {
                const entity = __fixtures__1.TEST_ENTITIES[0];
                const queryService = moduleRef.get(TestEntityService);
                return expect(queryService.setRelations('testReferences', entity._id.toHexString(), __fixtures__1.TEST_REFERENCES.slice(3, 6).map((r) => r._id.toHexString()), {
                    filter: { stringType: { eq: __fixtures__1.TEST_ENTITIES[1].stringType } },
                })).rejects.toThrow(`Unable to find TestEntity with id: ${String(entity._id)}`);
            });
            it('should throw an error if the relations are not found with the relationIds and provided filter', async () => {
    ...
    describe('#setRelation', () => {
        it('call select and return the result', async () => {
            ...
            expect(relation).toEqual(__fixtures__1.TEST_ENTITIES[1]);
        });
