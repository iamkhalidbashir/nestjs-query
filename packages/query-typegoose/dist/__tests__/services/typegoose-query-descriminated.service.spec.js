...
const test_discriminated_entity_1 = require("../__fixtures__/test-discriminated.entity");
const { Types } = typegoose_1.mongoose;
describe('TypegooseQueryService', () => {
    let moduleRef;
    let TestReferenceModel;
    let TestDiscriminatedEntityModel;
    ...
    TestReferenceService = (0, tslib_1.__decorate)([
        (0, tslib_1.__param)(0, (0, nestjs_typegoose_1.InjectModel)(__fixtures__1.TestReference)),
        (0, tslib_1.__metadata)("design:paramtypes", [Object])
    ], TestReferenceService);
    let TestDiscriminatedEntityService = class TestDiscriminatedEntityService extends services_1.TypegooseQueryService {
        constructor(model) {
            super(model);
            this.model = model;
    ...
    TestDiscriminatedEntityService = (0, tslib_1.__decorate)([
        (0, tslib_1.__param)(0, (0, nestjs_typegoose_1.InjectModel)(test_discriminated_entity_1.TestDiscriminatedEntity)),
        (0, tslib_1.__metadata)("design:paramtypes", [Object])
    ], TestDiscriminatedEntityService);
    beforeAll(async () => {
        moduleRef = await testing_1.Test.createTestingModule({
            imports: [
                nestjs_typegoose_1.TypegooseModule.forRoot(await (0, __fixtures__1.getConnectionUri)(), {
                    useFindAndModify: false,
                    useNewUrlParser: true,
                    useUnifiedTopology: true,
                }),
                src_1.NestjsQueryTypegooseModule.forFeature([
                    {
                        typegooseClass: __fixtures__1.TestEntity,
                        discriminators: [test_discriminated_entity_1.TestDiscriminatedEntity],
                    },
                    __fixtures__1.TestReference,
                ]),
            ],
    ...
    function testDiscriminatedEntityToObject(tde) {
        return {
            _id: tde._id,
    ...
    function testDiscriminatedEntityToCreate(tde) {
    ...
    beforeEach(async () => (0, __fixtures__1.prepareDb)());
    afterEach(async () => (0, __fixtures__1.dropDatabase)());
    describe('#query with discriminated entity', () => {
        it('call find and return the result', async () => {
            ...
            expect(convertDocuments(queryResult)).toEqual(expect.arrayContaining(__fixtures__1.TEST_DISCRIMINATED_ENTITIES));
        });
        it('should support eq operator', async () => {
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            const queryResult = await queryService.query({ filter: { stringType: { eq: 'foo11' } } });
            expect(convertDocuments(queryResult)).toEqual([__fixtures__1.TEST_DISCRIMINATED_ENTITIES[0]]);
        });
        it('should support neq operator', async () => {
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            const queryResult = await queryService.query({ filter: { stringType: { neq: 'foo1' } } });
            expect(convertDocuments(queryResult)).toEqual(expect.arrayContaining(__fixtures__1.TEST_DISCRIMINATED_ENTITIES.slice(1)));
        });
        it('should support gt operator', async () => {
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            const queryResult = await queryService.query({ filter: { numberType: { gt: 5 } } });
            expect(convertDocuments(queryResult)).toEqual(expect.arrayContaining(__fixtures__1.TEST_DISCRIMINATED_ENTITIES.slice(5)));
        });
        it('should support gte operator', async () => {
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            const queryResult = await queryService.query({ filter: { numberType: { gte: 5 } } });
            expect(convertDocuments(queryResult)).toEqual(expect.arrayContaining(__fixtures__1.TEST_DISCRIMINATED_ENTITIES.slice(4)));
        });
        it('should support lt operator', async () => {
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            const queryResult = await queryService.query({ filter: { numberType: { lt: 15 } } });
            expect(convertDocuments(queryResult)).toEqual(expect.arrayContaining(__fixtures__1.TEST_DISCRIMINATED_ENTITIES.slice(0, 4)));
        });
        it('should support lte operator', async () => {
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            const queryResult = await queryService.query({ filter: { numberType: { lte: 15 } } });
            expect(convertDocuments(queryResult)).toEqual(expect.arrayContaining(__fixtures__1.TEST_DISCRIMINATED_ENTITIES.slice(0, 5)));
        });
        it('should support in operator', async () => {
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            const queryResult = await queryService.query({ filter: { numberType: { in: [11, 12, 13] } } });
            expect(convertDocuments(queryResult)).toEqual(expect.arrayContaining(__fixtures__1.TEST_DISCRIMINATED_ENTITIES.slice(0, 3)));
        });
        ...
        it('should support is operator', async () => {
            ...
            expect(convertDocuments(queryResult)).toEqual(expect.arrayContaining(__fixtures__1.TEST_DISCRIMINATED_ENTITIES.filter((e) => e.boolType)));
        });
    ...
    describe('#aggregate with discriminated entity', () => {
            ...
            return expect(queryResult).toEqual([
                {
                    ...
                    max: {
                        dateType: __fixtures__1.TEST_DISCRIMINATED_ENTITIES[9].dateType,
                        numberType: 20,
                        stringType: 'foo20',
                        id: expect.any(Types.ObjectId),
                    },
                    min: {
                        dateType: __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0].dateType,
                        numberType: 11,
                        stringType: 'foo11',
                        id: expect.any(Types.ObjectId),
                    },
            ...
            return expect(queryResult).toEqual([
                {
                    ...
                    max: {
                        dateType: __fixtures__1.TEST_DISCRIMINATED_ENTITIES[8].dateType,
                        numberType: 19,
                        stringType: 'foo19',
                        id: expect.any(Types.ObjectId),
                    },
                ...
                },
                {
                    ...
                    min: {
                        dateType: __fixtures__1.TEST_DISCRIMINATED_ENTITIES[1].dateType,
                        numberType: 12,
                        stringType: 'foo12',
                        id: expect.any(Types.ObjectId),
                    },
        ...
        it('call select with the aggregate columns and return the result with a filter', async () => {
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            const queryResult = await queryService.aggregate({ stringType: { in: ['foo11', 'foo12', 'foo13'] } }, {
                count: ['id'],
                avg: ['numberType'],
                sum: ['numberType'],
                max: ['id', 'dateType', 'numberType', 'stringType'],
            ...
            return expect(queryResult).toEqual([
                {
                    ...
                    max: {
                        dateType: __fixtures__1.TEST_DISCRIMINATED_ENTITIES[2].dateType,
                        numberType: 13,
                        stringType: 'foo13',
                        id: expect.any(Types.ObjectId),
                    },
    ...
    describe('#count with discriminated entity', () => {
        it('should return number of elements matching a query', async () => {
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            const expectedEntities = __fixtures__1.TEST_DISCRIMINATED_ENTITIES.slice(0, 2);
    ...
    describe('#findById with discriminated entity', () => {
        it('return the entity if found', async () => {
            const entity = __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0];
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
        ...
        describe('with filter on discriminated entity', () => {
            ...
            it('should return an undefined if an entity with the pk and filter is not found', async () => {
                ...
                const queryService = moduleRef.get(TestDiscriminatedEntityService);
                const found = await queryService.findById(entity._id.toHexString(), {
                    filter: { stringType: { eq: __fixtures__1.TEST_DISCRIMINATED_ENTITIES[1].stringType } },
                });
                expect(found).toBeUndefined();
            });
        });
    ...
    describe('#createMany with discriminated entity', () => {
        it('call save on the repo with instances of entities when passed plain objects', async () => {
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            const created = await queryService.createMany(__fixtures__1.TEST_DISCRIMINATED_ENTITIES.map(testDiscriminatedEntityToCreate));
            expectEqualCreate(created, __fixtures__1.TEST_DISCRIMINATED_ENTITIES);
        });
        it('call save on the repo with instances of entities when passed instances', async () => {
            const instances = __fixtures__1.TEST_DISCRIMINATED_ENTITIES.map((e) => testDiscriminatedEntityToCreate(e));
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
        ...
        it('should reject if the entities already exist', async () => {
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            return expect(queryService.createMany(__fixtures__1.TEST_DISCRIMINATED_ENTITIES)).rejects.toThrow('Id cannot be specified when updating or creating');
        });
    });
    describe('#createOne with discriminated entity', () => {
        it('call save on the repo with an instance of the entity when passed a plain object', async () => {
            const entity = testDiscriminatedEntityToCreate(__fixtures__1.TEST_DISCRIMINATED_ENTITIES[0]);
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
        ...
        it('call save on the repo with an instance of the entity when passed an instance', async () => {
            const Model = (0, typegoose_1.getModelForClass)(test_discriminated_entity_1.TestDiscriminatedEntity);
            const entity = new Model(testDiscriminatedEntityToCreate(__fixtures__1.TEST_DISCRIMINATED_ENTITIES[0]));
    ...
    describe('#deleteMany with discriminated entity', () => {
        it('delete all records that match the query', async () => {
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            const entities = __fixtures__1.TEST_DISCRIMINATED_ENTITIES.slice(0, 5);
    ...
    describe('#deleteOne with discriminated entity', () => {
        it('remove the entity', async () => {
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            const deleted = await queryService.deleteOne(__fixtures__1.TEST_DISCRIMINATED_ENTITIES[0]._id.toHexString());
            expect(convertDocument(deleted)).toEqual(__fixtures__1.TEST_DISCRIMINATED_ENTITIES[0]);
        });
    ...
    describe('#updateMany with discriminated entity', () => {
        it('update all entities in the filter', async () => {
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            const filter = {
                stringType: { in: __fixtures__1.TEST_DISCRIMINATED_ENTITIES.slice(0, 5).map((e) => e.stringType) },
            };
        ...
        it('should reject if the update contains the ID', () => {
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            return expect(queryService.updateMany({ id: new Types.ObjectId().toHexString() }, {})).rejects.toThrow('Id cannot be specified when updating');
        });
        it('should not reject if the update contains an undefined id', async () => {
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            const entitiesToUpdate = __fixtures__1.TEST_DISCRIMINATED_ENTITIES.slice(0, 5);
            const filter = {
    ...
    describe('#updateOne with discriminated entity', () => {
        ...
        it('should reject if the update contains the ID', async () => {
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            return expect(queryService.updateOne(__fixtures__1.TEST_DISCRIMINATED_ENTITIES[0]._id.toHexString(), { _id: new Types.ObjectId() })).rejects.toThrow('Id cannot be specified when updating');
        });
        it('call fail if the entity is not found', async () => {
            const badId = new Types.ObjectId().toHexString();
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            return expect(queryService.updateOne(badId, { stringType: 'updated' })).rejects.toThrow(`Unable to find TestDiscriminatedEntity with id: ${badId}`);
        });
        describe('with filter on discriminated entity', () => {
            ...
            it('should throw an error if unable to find the entity', async () => {
                const entity = __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0];
                const queryService = moduleRef.get(TestDiscriminatedEntityService);
                return expect(queryService.updateOne(entity._id.toHexString(), { stringType: 'updated' }, { filter: { stringType: { eq: __fixtures__1.TEST_DISCRIMINATED_ENTITIES[1].stringType } } })).rejects.toThrow(`Unable to find TestDiscriminatedEntity with id: ${String(entity._id)}`);
            });
        });
    });
    describe('#findRelation with discriminated entity', () => {
        describe('with one entity', () => {
            it('call select and return the result', async () => {
                ...
                expect(queryResult).toEqual(__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[0]);
            });
            it('apply the filter option', async () => {
                ...
                const queryResult1 = await queryService.findRelation(__fixtures__1.TestReference, 'testReference', entity, {
                    filter: { referenceName: { eq: __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[0].referenceName } },
                });
                expect(queryResult1).toEqual(__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[0]);
                const queryResult2 = await queryService.findRelation(__fixtures__1.TestReference, 'testReference', entity, {
                    filter: { referenceName: { eq: __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[1].referenceName } },
                });
            ...
            it('throw an error if a relation with that name is not found.', async () => {
                const queryService = moduleRef.get(TestDiscriminatedEntityService);
                const entity = __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0];
                return expect(queryService.findRelation(__fixtures__1.TestReference, 'badReference', entity)).rejects.toThrow('Unable to find reference badReference on TestDiscriminatedEntity');
            });
            describe('virtual reference', () => {
                it('call select and return the result', async () => {
                    const entity = __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[0];
                    ...
                    expect(queryResult).toEqual(__fixtures__1.TEST_DISCRIMINATED_ENTITIES[0]);
                });
                it('apply the filter option', async () => {
                    const entity = __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[0];
                    const queryService = moduleRef.get(TestReferenceService);
                    const queryResult1 = await queryService.findRelation(__fixtures__1.TestEntity, 'virtualTestEntity', entity, {
                        filter: { stringType: { eq: __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0].stringType } },
                    });
                    expect(queryResult1).toEqual(__fixtures__1.TEST_DISCRIMINATED_ENTITIES[0]);
        ...
        describe('with multiple discriminated entities', () => {
            it('call select and return the result', async () => {
                const entities = __fixtures__1.TEST_DISCRIMINATED_ENTITIES.slice(0, 3);
                const queryService = moduleRef.get(TestDiscriminatedEntityService);
                ...
                expect(queryResult).toEqual(new Map([
                    [entities[0], expect.objectContaining(__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[0])],
                    [entities[1], expect.objectContaining(__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[3])],
                    [entities[2], expect.objectContaining(__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[6])],
                ]));
            });
            it('should apply the filter option', async () => {
                ...
                const options = {
                    filter: {
                        _id: { in: [__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[0]._id, __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[6]._id] },
                    },
                };
            ...
            it('should return undefined select if no results are found.', async () => {
                const entities = [
                    __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0],
    ...
    describe('#queryRelations on discriminated entities', () => {
        describe('with one entity', () => {
            it('call select and return the result', async () => {
                const queryService = moduleRef.get(TestDiscriminatedEntityService);
                const queryResult = await queryService.queryRelations(__fixtures__1.TestReference, 'testReferences', __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0], {
                    filter: { referenceName: { isNot: null } },
                });
                return expect(queryResult).toEqual(__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(0, 3));
            });
            it('should apply a filter', async () => {
                ...
                expect(queryResult).toEqual([__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[1]]);
            });
            it('should apply paging', async () => {
                ...
                expect(queryResult).toEqual(__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(1, 3));
            });
        });
        describe('with virtual discriminated entity', () => {
            it('call select and return the result', async () => {
                const queryService = moduleRef.get(TestDiscriminatedEntityService);
                const queryResult = await queryService.queryRelations(__fixtures__1.TestReference, 'virtualTestReferences', __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0], {
                    filter: { referenceName: { isNot: null } },
                });
                return expect(queryResult).toEqual(expect.arrayContaining(__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(0, 3)));
            });
            it('should apply a filter', async () => {
                const queryService = moduleRef.get(TestDiscriminatedEntityService);
                const queryResult = await queryService.queryRelations(__fixtures__1.TestReference, 'virtualTestReferences', __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0], {
        ...
        describe('with multiple discriminated entities', () => {
            it('call return a map of results for each entity', async () => {
                ...
                expect(queryResult.get(entities[0])).toEqual(__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(0, 3));
                expect(queryResult.get(entities[1])).toEqual(__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(3, 6));
                expect(queryResult.get(entities[2])).toEqual(__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(6, 9));
            });
            it('should apply a filter per entity', async () => {
                const entities = __fixtures__1.TEST_DISCRIMINATED_ENTITIES.slice(0, 3);
                const references = [
                    __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[1],
                    __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[4],
                    __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[7],
                ];
                const queryService = moduleRef.get(TestDiscriminatedEntityService);
                const queryResult = await queryService.queryRelations(__fixtures__1.TestReference, 'testReferences', entities, {
            ...
            it('should apply paging per entity', async () => {
                ...
                expect(queryResult.size).toBe(3);
                expect(queryResult.get(entities[0])).toEqual(__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(1, 3));
                expect(queryResult.get(entities[1])).toEqual(__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(4, 6));
                expect(queryResult.get(entities[2])).toEqual(__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(7, 9));
            });
    ...
    describe('#aggregateRelations', () => {
        describe('with one discriminated entity', () => {
            it('should return an aggregate', async () => {
                const queryService = moduleRef.get(TestDiscriminatedEntityService);
                const aggResult = await queryService.aggregateRelations(__fixtures__1.TestReference, 'testReferences', __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0], { referenceName: { isNot: null } }, { count: ['id'] });
            ...
            it('should support groupBy when aggregating relations', async () => {
                const queryService = moduleRef.get(TestDiscriminatedEntityService);
                const aggResult = await queryService.aggregateRelations(__fixtures__1.TestReference, 'testReferences', __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0], { referenceName: { isNot: null } }, { groupBy: ['testEntity'], count: ['id'] });
                return expect(aggResult).toEqual([
                    {
                        groupBy: { testEntity: __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0]._id },
                        count: {
                            id: 3,
                        },
                    },
        ...
        describe('with virtual relation', () => {
            it('call select and return the result', async () => {
                const queryService = moduleRef.get(TestDiscriminatedEntityService);
                const aggResult = await queryService.aggregateRelations(__fixtures__1.TestReference, 'virtualTestReferences', __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0], { referenceName: { isNot: null } }, { count: ['id'] });
                return expect(aggResult).toEqual([
                    {
                        count: {
                            id: 3,
                        ...
                        [
                            {
                                ...
                                max: {
                                    referenceName: __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[2].referenceName,
                                    testEntity: entities[0]._id,
                                    id: expect.any(Types.ObjectId),
                                },
                                min: {
                                    referenceName: __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[0].referenceName,
                                    testEntity: entities[0]._id,
                                    id: expect.any(Types.ObjectId),
                                },
                            },
                        ...
                        [
                            {
                                ...
                                max: {
                                    referenceName: __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[5].referenceName,
                                    testEntity: entities[1]._id,
                                    id: expect.any(Types.ObjectId),
                                },
                                min: {
                                    referenceName: __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[3].referenceName,
                                    testEntity: entities[1]._id,
                                    id: expect.any(Types.ObjectId),
                                },
                            },
                        ...
                        [
                            {
                                ...
                                max: {
                                    referenceName: __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[8].referenceName,
                                    testEntity: entities[2]._id,
                                    id: expect.any(Types.ObjectId),
                                },
                                min: {
                                    referenceName: __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[6].referenceName,
                                    testEntity: entities[2]._id,
                                    id: expect.any(Types.ObjectId),
                                },
                            },
        ...
        describe('with one entity', () => {
            it('count the references', async () => {
                ...
                const countResult = await queryService.countRelations(__fixtures__1.TestReference, 'testReferences', entity, {
                    referenceName: {
                        in: [
                            __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[1].referenceName,
                            __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[2].referenceName,
                        ],
                    },
                });
                ...
                const queryResult = await queryService.countRelations(__fixtures__1.TestReference, 'testReferences', entities, {
                    referenceName: {
                        in: [
                            __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[1].referenceName,
                            __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[2].referenceName,
                            __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[4].referenceName,
                            __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[5].referenceName,
                            __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[7].referenceName,
                            __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[8].referenceName,
                        ],
                    },
                });
                expect(queryResult).toEqual(new Map([
    ...
    describe('#addRelations', () => {
        it('call select and return the result', async () => {
            const entity = __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0];
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            const queryResult = await queryService.addRelations('testReferences', entity._id.toHexString(), __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(3, 6).map((r) => r._id.toHexString()));
            expect(queryResult).toEqual(expect.objectContaining({
                _id: entity._id,
                testReferences: expect.arrayContaining(__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(0, 6).map((r) => r._id)),
            }));
        ...
        it('should not modify relations if relationIds is empty', async () => {
            ...
            expect(queryResult).toEqual(expect.objectContaining({
                _id: entity._id,
                testReferences: expect.arrayContaining(__fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(0, 3).map((r) => r._id)),
            }));
            const relations = await queryService.queryRelations(__fixtures__1.TestReference, 'testReferences', entity, {});
        ...
        describe('with virtual reference', () => {
            it('should return a rejected promise', async () => {
                const entity = __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0];
                const queryService = moduleRef.get(TestDiscriminatedEntityService);
                return expect(queryService.addRelations('virtualTestReferences', entity._id.toHexString(), __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(3, 6).map((r) => r._id.toHexString()))).rejects.toThrow('AddRelations not supported for virtual relation virtualTestReferences');
            });
        });
        describe('with modify options', () => {
            it('should throw an error if the entity is not found with the id and provided filter', async () => {
                const entity = __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0];
                const queryService = moduleRef.get(TestDiscriminatedEntityService);
                return expect(queryService.addRelations('testReferences', entity._id.toHexString(), __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(3, 6).map((r) => r._id.toHexString()), {
                    filter: { stringType: { eq: __fixtures__1.TEST_DISCRIMINATED_ENTITIES[1].stringType } },
                })).rejects.toThrow(`Unable to find TestDiscriminatedEntity with id: ${String(entity._id)}`);
            });
    ...
    describe('#setRelations', () => {
        it('set all relations on the entity', async () => {
            const entity = __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0];
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            const relationIds = __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(3, 6).map((r) => r._id);
            const queryResult = await queryService.setRelations('testReferences', entity._id.toHexString(), relationIds.map((id) => id.toHexString()));
            expect(queryResult).toEqual(expect.objectContaining({
                _id: entity._id,
        ...
        describe('with modify options', () => {
            it('should throw an error if the entity is not found with the id and provided filter', async () => {
                const entity = __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0];
                const queryService = moduleRef.get(TestDiscriminatedEntityService);
                return expect(queryService.setRelations('testReferences', entity._id.toHexString(), __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(3, 6).map((r) => r._id.toHexString()), {
                    filter: { stringType: { eq: __fixtures__1.TEST_DISCRIMINATED_ENTITIES[1].stringType } },
                })).rejects.toThrow(`Unable to find TestDiscriminatedEntity with id: ${String(entity._id)}`);
            });
            it('should throw an error if the relations are not found with the relationIds and provided filter', async () => {
    ...
    describe('#setRelation', () => {
        it('call select and return the result', async () => {
            const entity = __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[0];
            const queryService = moduleRef.get(TestReferenceService);
            const queryResult = await queryService.setRelation('testEntity', entity._id.toHexString(), __fixtures__1.TEST_DISCRIMINATED_ENTITIES[1]._id.toHexString());
            expect(queryResult).toEqual(expect.objectContaining({ ...entity, testEntity: __fixtures__1.TEST_DISCRIMINATED_ENTITIES[1]._id }));
            const relation = await queryService.findRelation(__fixtures__1.TestEntity, 'testEntity', entity);
            expect(relation).toEqual(__fixtures__1.TEST_DISCRIMINATED_ENTITIES[1]);
        });
        it('should reject with a virtual reference', async () => {
            const entity = __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[0];
            const queryService = moduleRef.get(TestReferenceService);
            return expect(queryService.setRelation('virtualTestEntity', entity._id.toHexString(), __fixtures__1.TEST_DISCRIMINATED_ENTITIES[1]._id.toHexString())).rejects.toThrow('SetRelation not supported for virtual relation virtualTestEntity');
        });
        describe('with modify options', () => {
            it('should throw an error if the entity is not found with the id and provided filter', async () => {
                const entity = __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[0];
                const queryService = moduleRef.get(TestReferenceService);
                return expect(queryService.setRelation('testEntity', entity._id.toHexString(), __fixtures__1.TEST_DISCRIMINATED_ENTITIES[1]._id.toHexString(), {
                    filter: { referenceName: { eq: __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[1].referenceName } },
            ...
            it('should throw an error if the relations are not found with the relationIds and provided filter', async () => {
                const entity = __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[0];
                const queryService = moduleRef.get(TestReferenceService);
                return expect(queryService.setRelation('TestDiscriminatedEntity', entity._id.toHexString(), __fixtures__1.TEST_DISCRIMINATED_ENTITIES[1]._id.toHexString(), {
    ...
    describe('#removeRelation', () => {
        it('call select and return the result', async () => {
            const entity = __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[0];
            const queryService = moduleRef.get(TestReferenceService);
            const queryResult = await queryService.removeRelation('testEntity', entity._id.toHexString(), __fixtures__1.TEST_DISCRIMINATED_ENTITIES[1]._id.toHexString());
        ...
        it('should reject with a virtual reference', async () => {
            const entity = __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[0];
            const queryService = moduleRef.get(TestReferenceService);
            return expect(queryService.removeRelation('virtualTestEntity', entity._id.toHexString(), __fixtures__1.TEST_DISCRIMINATED_ENTITIES[1]._id.toHexString())).rejects.toThrow('RemoveRelation not supported for virtual relation virtualTestEntity');
        });
        describe('with modify options', () => {
            it('should throw an error if the entity is not found with the id and provided filter', async () => {
                const entity = __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[0];
                const queryService = moduleRef.get(TestReferenceService);
                return expect(queryService.removeRelation('testEntity', entity._id.toHexString(), __fixtures__1.TEST_DISCRIMINATED_ENTITIES[1]._id.toHexString(), {
                    filter: { referenceName: { eq: __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[1].referenceName } },
                })).rejects.toThrow(`Unable to find TestReference with id: ${String(entity._id)}`);
            });
            it('should throw an error if the relations are not found with the relationIds and provided filter', async () => {
                const entity = __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES[0];
                const queryService = moduleRef.get(TestReferenceService);
                return expect(queryService.removeRelation('TestDiscriminatedEntity', entity._id.toHexString(), __fixtures__1.TEST_DISCRIMINATED_ENTITIES[1]._id.toHexString(), {
                    relationFilter: { stringType: { like: '%-one' } },
                })).rejects.toThrow('Unable to find reference TestDiscriminatedEntity on TestReference');
            });
        });
    ...
    describe('#removeRelations', () => {
        it('call select and return the result', async () => {
            const entity = __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0];
            const queryService = moduleRef.get(TestDiscriminatedEntityService);
            const queryResult = await queryService.removeRelations('testReferences', entity._id.toHexString(), __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(0, 3).map((r) => r._id.toHexString()));
        ...
        describe('with virtual reference', () => {
            it('should return a rejected promise', async () => {
                const entity = __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0];
                const queryService = moduleRef.get(TestDiscriminatedEntityService);
                return expect(queryService.removeRelations('virtualTestReferences', entity._id.toHexString(), __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(0, 3).map((r) => r._id.toHexString()))).rejects.toThrow('RemoveRelations not supported for virtual relation virtualTestReferences');
            });
        });
        describe('with modify options', () => {
            it('should throw an error if the entity is not found with the id and provided filter', async () => {
                const entity = __fixtures__1.TEST_DISCRIMINATED_ENTITIES[0];
                const queryService = moduleRef.get(TestDiscriminatedEntityService);
                return expect(queryService.removeRelations('testReferences', entity._id.toHexString(), __fixtures__1.TEST_REFERENCES_FOR_DISCRIMINATES.slice(0, 3).map((r) => r._id.toHexString()), {
                    filter: { stringType: { eq: __fixtures__1.TEST_DISCRIMINATED_ENTITIES[1].stringType } },
                })).rejects.toThrow(`Unable to find TestDiscriminatedEntity with id: ${String(entity._id)}`);
            });
            it('should throw an error if the relations are not found with the relationIds and provided filter', async () => {
...
//# sourceMappingURL=typegoose-query-descriminated.service.spec.js.map