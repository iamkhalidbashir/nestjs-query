...
const documents = [todo_item_entity_1.TodoItemEntity.name, sub_task_entity_1.SubTaskEntity.name, tag_entity_1.TagEntity.name];
const truncate = async (connection) => (0, helpers_1.asyncLoop)(documents, (document) => connection.model(document).remove({}).exec());
exports.truncate = truncate;
const refresh = async (connection) => {
    await (0, exports.truncate)(connection);
    const TodoModel = connection.model(todo_item_entity_1.TodoItemEntity.name);
    const TagsModel = connection.model(tag_entity_1.TagEntity.name);
    const SubTaskModel = connection.model(sub_task_entity_1.SubTaskEntity.name);
