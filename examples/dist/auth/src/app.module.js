"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.AppModule = void 0;
const tslib_1 = require("tslib");
const common_1 = require("@nestjs/common");
const graphql_1 = require("@nestjs/graphql");
const typeorm_1 = require("@nestjs/typeorm");
const tag_module_1 = require("./tag/tag.module");
const todo_item_module_1 = require("./todo-item/todo-item.module");
const sub_task_module_1 = require("./sub-task/sub-task.module");
const helpers_1 = require("../../helpers");
const auth_module_1 = require("./auth/auth.module");
const user_module_1 = require("./user/user.module");
let AppModule = class AppModule {
};
AppModule = (0, tslib_1.__decorate)([
    (0, common_1.Module)({
        imports: [
            typeorm_1.TypeOrmModule.forRoot((0, helpers_1.typeormOrmConfig)('auth')),
            graphql_1.GraphQLModule.forRoot({
                autoSchemaFile: 'schema.gql',
                installSubscriptionHandlers: true,
                subscriptions: {
                    'subscriptions-transport-ws': {
                        onConnect: (connectionParams) => ({ headers: connectionParams }),
                    },
                },
                context: ({ req, connection }) => ({ req: { ...req, ...connection === null || connection === void 0 ? void 0 : connection.context } }),
            }),
            auth_module_1.AuthModule,
            user_module_1.UserModule,
            todo_item_module_1.TodoItemModule,
            sub_task_module_1.SubTaskModule,
            tag_module_1.TagModule,
        ],
    })
], AppModule);
exports.AppModule = AppModule;
//# sourceMappingURL=app.module.js.map